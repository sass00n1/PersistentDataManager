...
//rigidbody2D的相关设置：
//1.碰撞检测方式：连续的。
//2.Interpolate(定义如何在物理更新间隔之间插入游戏对象的移动,运动趋于颠簸状态时很有用)：Interpolate(根据游戏对象在先前帧中的位置来平滑移动)
//3.跳跃不合适时，可以调节重力和起跳力度
public class CharacterController2D : MonoBehaviour
{
    ...
    private bool m_Grounded;                      // Whether or not the player is grounded. 玩家是否接地
    ...
    public void Move(float move, bool crouch, bool jump)
    {
        ...
        if (m_Grounded && jump)
        {
            ...
            //m_Grounded = false; //问题：这里跳跃之后，会立刻变成离地状态，然后FixedUpdate在脚底发射线，触碰到地面，触发落地事件。。。
